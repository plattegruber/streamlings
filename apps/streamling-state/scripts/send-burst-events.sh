#!/bin/bash
# Simulate an active stream with burst of events (direct to streamling-state)

echo "🎉 Simulating active stream chat (burst mode - direct)!"
echo "   Watch the energy and mood change in the logs..."
echo ""
...
for round in {1..6}; do
  ...
  if [ $((RANDOM % 5)) -lt 2 ]; then
    ...
    twitch event trigger "$random_hv" -F http://localhost:8787/webhook > /dev/null 2>&1 &
  fi
...
echo ""
echo "✅ Burst complete! Check the logs above to see mood transitions."
